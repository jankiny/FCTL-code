...
def get_aircraft_datasets(train_transform, test_transform, train_classes=range(60),
                       open_set_classes=range(60, 100), balance_open_set_eval=False,
                          split_train_val=True, seed=0, data_path='./data'):

    np.random.seed(seed)
    root = os.path.join(data_path, 'fgvc-aircraft-2013b')
    # Init train dataset and subsample training classes
    train_dataset_whole = FGVCAircraft(root=root, transform=train_transform, split='trainval')
    ...
    test_dataset_known = FGVCAircraft(root=root, transform=test_transform, split='test')
    test_dataset_known = subsample_classes(test_dataset_known, include_classes=train_classes)

    # Get testset for unknown classes
    test_dataset_unknown = FGVCAircraft(root=root, transform=test_transform, split='test')
